...
const colors: Record<string, string> = { 
  basic: "text-muted-foreground dark:text-muted-foreground", 
  silver: "text-blue-600 dark:text-info", 
  gold: "text-amber-600 dark:text-warning", 
  platinum: "text-info" 
};
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {plans.map((plan) => {
        ...
        return <Card key={plan._id} className="p-5 bg-card border-border/40 flex flex-col">
          ...
          <div className="flex gap-3 text-xs text-muted-foreground my-3"><span className="flex gap-1"><Users className="w-3 h-3" />{plan.activeSubscribers || 0} نشط</span><span>{plan.subscribers || 0} إجمالي</span><span>{Number(plan.revenue || 0).toLocaleString("ar-SY")} ل.س</span></div>
          ...
          <div className="flex gap-2 mt-4"><Button variant="outline" size="sm" className="flex-1 gap-2" onClick={() => onEdit(plan)}><Edit2 className="w-3.5 h-3.5" />تعديل</Button>{plan.isActive && <Button variant="outline" size="icon-sm" className="text-danger" onClick={() => onDeleteClick(plan._id)}><Trash2 className="w-3.5 h-3.5" /></Button>}</div>
        </Card>;
      })}
    </div>
  );
