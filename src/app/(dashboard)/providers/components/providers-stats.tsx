...
const cityChart = {
      ...
      return `<div style="display:flex; flex-direction:column; gap:6px;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; gap:20px;">
          <span style="display:flex; align-items:center; gap:6px; color:#94a3b8">
            <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#10b981; box-shadow:0 0 8px #10b981"></span>مزودو الخدمة
          </span>
          <b style="color:#6ee7b7; font-variant-numeric:tabular-nums">${p.value.toLocaleString("ar-EG")} مزود</b>
        </div>
      </div>`;
    },
  },
  ...
  series: [{
    ...
    barWidth: "48%",
    barMaxWidth: 24,
    label: { show: true, position: "right", color: "#94a3b8", fontSize: 10, fontWeight: "bold", offset: [6, 0] },
...
const emergencyDonut = {
  ...
  series: [{
    ...
    center: ["50%", "50%"],
    data: [
      { name: "طوارئ", value: emergencyProv, itemStyle: { color: "#ef4444", borderColor: "#0d0916", borderWidth: 3 } },
      { name: "عادي", value: normalProv, itemStyle: { color: "#3b82f6", borderColor: "#0d0916", borderWidth: 3 } },
    ],
    label: { show: false },
...
const verifiedDonut = {
  ...
  series: [{
    ...
    center: ["50%", "50%"],
    data: [
      { name: "موثق", value: verifiedProv, itemStyle: { color: "#10b981", borderColor: "#0d0916", borderWidth: 3 } },
      { name: "غير موثق", value: unverifiedProv, itemStyle: { color: "#475569", borderColor: "#0d0916", borderWidth: 3 } },
    ],
    label: { show: false },
    emphasis: { scale: true, scaleSize: 6, itemStyle: { shadowBlur: 20, shadowColor: "rgba(143,92,177,0.3)" } },
  }],
  ...
  tooltip: {
    ...
    formatter: (params: any[]) => {
      ...
      return `<div style="min-width:140px">
        <b style="color:#f5f5f7;display:block;margin-bottom:8px">${open?.axisValue}</b>
        <div style="display:flex;justify-content:space-between;gap:16px;margin-bottom:5px">
          <span style="color:#94a3b8"><span style="width:8px;height:8px;border-radius:2px;background:#10b981;display:inline-block;margin-left:4px"></span>مفتوح</span>
          <b style="color:#6ee7b7">${(open?.value || 0).toLocaleString("ar-EG")}</b>
        </div>
        <div style="display:flex;justify-content:space-between;gap:16px;margin-bottom:5px">
          <span style="color:#94a3b8"><span style="width:8px;height:8px;border-radius:2px;background:#ef4444;display:inline-block;margin-left:4px"></span>مغلق</span>
          <b style="color:#fca5a5">${(closed?.value || 0).toLocaleString("ar-EG")}</b>
        </div>
  ...
  series: [
    {
      ...
      itemStyle: {
        color: makeGradientBar("#10b981", "rgba(16,185,129,0.5)"),
      },
      barWidth: "48%",
      barMaxWidth: 35,
    },
    {
      ...
      itemStyle: {
        color: makeGradientBar("#ef4444", "rgba(239,68,68,0.5)"),
...
const avgHoursChart = {
  ...
  tooltip: {
    ...
    formatter: (params: any[]) => {
      const p = params[0];
      return `<div><b style="color:#f5f5f7">${p.axisValue}</b><br/>
        <span style="color:#6366f1">● متوسط ساعات العمل: </span><b style="color:#a5b4fc">${p.value} ساعة</b></div>`;
    },
  },
  ...
  series: [{
    ...
    barWidth: "48%",
    barMaxWidth: 35,
    label: { show: true, position: "top", color: "#94a3b8", fontSize: 10, fontWeight: "bold", formatter: "{c}h" },
    emphasis: { itemStyle: { shadowBlur: 16, shadowColor: "rgba(0,0,0,0.4)" } },
  }],
};
  ...
  series: [
    ...
    },
    {
      ...
      data: [{
        ...
        lineStyle: { color: "#10b981", width: 2, shadowColor: "rgba(16,185,129,0.4)", shadowBlur: 6 },
        areaStyle: {
          color: { type: "radial", x: 0.5, y: 0.5, r: 0.5,
...
const approvalGaugeOption = {
  ...
  series: [{
    ...
    progress: {
      ...
      itemStyle: {
        color: {
          ...
          colorStops: [{ offset: 0, color: "#8f5cb1" }, { offset: 1, color: "#a57ed8" }],
        },
      },
    },
    pointer: { show: false },
    ...
    title: { offsetCenter: [0, "25%"], color: "#64748b", fontSize: 10, fontFamily: "IBM Plex Sans Arabic" },
    detail: {
      valueAnimation: true,
      offsetCenter: [0, "-5%"],
      fontSize: 28,
...
export function ProvidersStats({ summary, isLoading = false }: ProvidersStatsProps) {
  ...
  const emergencyDonutOption = {
    ...
    series: [{
      ...
      data: [
        { name: "Ø·ÙˆØ§Ø±Ø¦", value: emergencyProviders, itemStyle: { color: "#ef4444", borderColor: "#0d0916", borderWidth: 3 } },
        { name: "Ø¹Ø§Ø¯ÙŠ", value: normalProviders, itemStyle: { color: "#3b82f6", borderColor: "#0d0916", borderWidth: 3 } },
      ],
    }],
  };
  ...
  const verifiedDonutOption = {
    ...
    series: [{
      ...
      data: [
        { name: "Ù…ÙˆØ«Ù‚", value: verifiedProviders, itemStyle: { color: "#10b981", borderColor: "#0d0916", borderWidth: 3 } },
        { name: "ØºÙŠØ± Ù…ÙˆØ«Ù‚", value: unverifiedProviders, itemStyle: { color: "#475569", borderColor: "#0d0916", borderWidth: 3 } },
      ],
    }],
  };
  ...
  if (isLoading) {
    return <Card className="p-5 bg-card border-border/40 text-sm text-muted-foreground">جاري تحميل تحليلات المزودين من الخادم...</Card>;
  }
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="p-5 bg-card/70 backdrop-blur-xl border border-border/40 hover:border-border/70 transition-all duration-300 group relative overflow-hidden flex flex-col justify-between">
            <div className="absolute inset-0 bg-gradient-to-br from-primary/[0.03] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
            <div className="mb-4 relative z-10">
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center">
              ...
              <div className="flex justify-center gap-6 mt-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-muted-foreground/70">طوارئ</span>
                  <span className="text-[11px] font-bold text-white tabular-nums">{emergencyProviders.toLocaleString("ar-EG")}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: "#3b82f6", boxShadow: `0 0 6px #3b82f660` }} />
                  <span className="text-[10px] text-muted-foreground/70">عادي</span>
                  <span className="text-[11px] font-bold text-white tabular-nums">{normalProviders.toLocaleString("ar-EG")}</span>
                </div>
              </div>
            </div>
          </Card>
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center">
              ...
              <div className="flex justify-center gap-6 mt-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-muted-foreground/70">موثق</span>
                  <span className="text-[11px] font-bold text-white tabular-nums">{verifiedProviders.toLocaleString("ar-EG")}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: "#475569", boxShadow: `0 0 6px #47556960` }} />
                  <span className="text-[10px] text-muted-foreground/70">غير موثق</span>
                  <span className="text-[11px] font-bold text-white tabular-nums">{unverifiedProviders.toLocaleString("ar-EG")}</span>
                </div>
              </div>
            </div>
          </Card>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <Card className="p-4 bg-card/60 border-border/40 flex items-center gap-4 hover:border-border/70 transition-all">
            ...
            <div>
              <p className="text-xl font-black text-emerald-400 tabular-nums">{workingHours.fridayOpen.toLocaleString()}</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">مفتوح الجمعة</p>
            </div>
          </Card>
          ...
          <Card className="p-4 bg-card/60 border-border/40 flex items-center gap-4 hover:border-border/70 transition-all">
            ...
            <div>
              <p className="text-xl font-black text-rose-400 tabular-nums">{workingHours.fridayClosed.toLocaleString()}</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">مغلق الجمعة</p>
            </div>
          </Card>
          ...
          <Card className="p-4 bg-card/60 border-border/40 flex items-center gap-4 hover:border-border/70 transition-all">
            ...
            <div>
              <p className="text-xl font-black text-blue-400 tabular-nums">{workingHours.allDay24h.toLocaleString()}</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">24 ساعة طوال الأسبوع</p>
            </div>
          </Card>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {jsonFields.map((f: any) => (
              <div key={f.field} className="flex items-center justify-between py-2 px-3 rounded-xl bg-secondary/30 hover:bg-secondary/50 transition-colors border border-border/30 hover:border-border/60">
                <span className="text-xs text-blue-400 font-mono">{f.field}</span>
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-bold ${f.failed > 0 ? "text-amber-400" : "text-emerald-400"}`}>
                    {f.parsed}/{f.nonNull}
                  </span>
                  <span className="text-[10px] text-muted-foreground/50 font-mono">avg:{f.avgItems}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {insights.map((ins: any, i: number) => (
              <div key={i} className="flex items-start gap-3 py-2.5 px-4 rounded-xl bg-secondary/20 hover:bg-secondary/40 transition-all border border-transparent hover:border-border/30">
                <span className={`shrink-0 mt-0.5 px-2 py-0.5 rounded text-[10px] font-bold ${
                  ins.priority === "HIGH" ? "bg-red-500/15 text-red-400 border border-red-500/20" :
                  ins.priority === "MED" ? "bg-amber-500/15 text-amber-400 border border-amber-500/20" :
