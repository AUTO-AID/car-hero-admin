...
export function ServicesList({
  ...
  if (services.length === 0) {
    return (
      <Card className="p-16 bg-card/60 border-border/40">
        <EmptyState
          icon={AlertCircle}
          title="لا توجد خدمات مطابقة"
          description="غيّر البحث أو الفلاتر لعرض خدمات أخرى."
        />
      </Card>
    );
  }
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                {isEmergency && (
                  <Badge className="bg-rose-500/10 text-danger border-rose-500/20 text-xs px-1.5 font-bold uppercase">
                    طارئ
                  </Badge>
                )}
                <Switch
            ...
            <p className="text-xs text-muted-foreground truncate mb-2">{service.name}</p>
            <Badge variant="outline" className={`text-xs px-2 py-0 border-transparent mb-4 ${cat.bg} ${cat.color} font-bold`}>
              {cat.label}
            </Badge>
            ...
            <div className="grid grid-cols-2 gap-2 mb-3 bg-secondary/20 p-3 rounded-xl border border-border/30">
              <div>
                ...
                <p className="text-xs font-bold text-white tabular-nums">
                  ...
                  <span className="text-xs font-normal text-muted-foreground/60 ms-1">ل.س</span>
                </p>
              </div>
              <div className="text-end">
                <p className="text-xs text-muted-foreground mb-0.5">المدة</p>
                <p className="text-xs font-bold text-white tabular-nums flex items-center justify-end gap-1">
                  <Clock className="w-3 h-3 text-muted-foreground/60" />
                  {service.estimatedDuration}
                  <span className="text-xs font-normal text-muted-foreground/60">دقيقة</span>
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <div className="rounded-lg border border-border/25 bg-background/30 px-2.5 py-2">
                ...
                <p className="text-xs font-bold text-white tabular-nums">{Number(service.ordersCount || 0).toLocaleString("ar-SA")}</p>
              </div>
              <div className="rounded-lg border border-border/25 bg-background/30 px-2.5 py-2">
                <p className="text-xs text-muted-foreground flex items-center gap-1"><ReceiptText className="w-3 h-3" /> إيراد</p>
                <p className="text-xs font-bold text-primary tabular-nums">{Number(service.ordersRevenue || 0).toLocaleString("ar-SA")}</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => onDelete(serviceId)}
                className="h-8 w-8 p-0 border-rose-500/20 text-danger hover:bg-rose-500/10 hover:text-danger transition-all shrink-0">
